...
!      write(*,*) stack
    ...
    do i = 1, N 
      if (connections(i,id) == .true.) then ! if connected
        if(sum(findloc(stack, caveids(i))) == 0 .or. is_cap(caveids(i)(1:1)) .or. (maxval(counts) < 2 .and. caveids(i) /= "start" .and. caveids(i) /= "end")) then
            if(.not. is_cap(caveids(i)(1:1)) .and. caveids(i) /= "start" .and. caveids(i) /= "end") then
              counts(i) = counts(i) + 1
            end if
